...
The legacy horizontal Personal picks shelf is no longer part of the YouTube runtime. Once native-card enforcement and safe replacement slots are active, MyAlgo uses a single coherent feed surface: YouTube's native grid/list, annotated and selectively replaced in place. This avoids presenting a second horizontal feed that could be mistaken for the primary recommendation surface.
