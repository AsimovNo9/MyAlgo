...
| Semantic-path recommendation explanations | Verified locally | Feed items expose approved concept paths with relation types and bounded confidence, and feed cache persists the path without raw prompts or vector distances; production migration/application verification remains open. |
| Reviewed semantic-context ingestion | Verified locally | CRON-authorized ingestion validates and idempotently upserts versioned pending catalog entries with provenance; approved/rejected statuses are preserved and embeddings remain a separate model-versioned backfill step. |
