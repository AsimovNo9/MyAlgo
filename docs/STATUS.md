...
| Per-interest candidate coverage | Verified locally | Discovery gating now counts coverage per strong topic and triggers when any interest is underrepresented; production sync metrics remain open. |
...
| Embeddings / pgvector retrieval | Partially verified in production | pgvector tables, HNSW index, and `match_content_embeddings` RPC return `200`; one real embedding was generated locally against production Supabase/OpenAI. Vercel provider configuration and deployed backfill remain unverified. |
