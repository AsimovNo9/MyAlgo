...
Disable Vercel Git integration's automatic production deployments from `main`; otherwise Vercel will continue deploying every main push in parallel with the tag workflow. Add GitHub Actions production secrets `VERCEL_TOKEN`, `VERCEL_ORG_ID`, and `VERCEL_PROJECT_ID`. Keep application secrets in Vercel's production environment variables.
